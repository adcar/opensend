class DocumentsController < ApplicationController
  ...
  def share_link_params
    ...
    # Set expiration to end of day if date provided, enforce max 30 days
    if permitted[:expires_at].present?
      date = Date.parse(permitted[:expires_at]) rescue nil
      if date
        max_date = Date.today + ShareLink::MAX_EXPIRATION_DAYS.days
        date = [date, max_date].min
        permitted[:expires_at] = date.end_of_day
      end
    else
      # Expiration is required, default to 30 days
      permitted[:expires_at] = ShareLink::MAX_EXPIRATION_DAYS.days.from_now.end_of_day
    end
